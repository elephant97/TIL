...
* Heap영역 (GC가 이 영역의 참조가 없어질 떄 정리함)
...
* 메모리 릭이 발생할 수 있음
  * static은 메서드 영역에 할당 되는데, 객체가 생성되고, GC에 의해 정리될 떄 Static은 정리되지 않는 영역이므로 메모리 누수를 초래할 수 있다.
...
### float과 double형은 왜 계산시 오차를 발생 시키나?
* float과 double은 부동 소수점(floating-point) 형식을 사용하는 데이터 타입으로, 실수를 근사적으로 표현함.
* 유한한 비트 수로 무한한 실수를 표현하려고 하는 데에 한계가 있기 때문에, 특히 소수점 이하 자릿수가 많은 경우나 무리수와 같이 정확한 이진 표현이 어려운 숫자들을 다룰 때 부정확성이 발생할 수 있다.
 ...
 * Java ex) String A = new String("a");
 *          String copy = A;
 * C ex) char *A = "a";
 *       char *copy = A;

### BigInt는 어떻게 구현되나?
* 결론적으로 BigInteger는 int 배열로 만들어짐!
